...
public class DiFileInputStream extends FileInputStream {
	private int _location; // position in the file, equal to the number of bytes
							// read
	private boolean _little_endian;

	/**
	 * Default Constructor, argument is a string containing the file name.
	 * Endianess is little by default.
	 * 
	 * @param fname
	 *            the filename
	 * @throws IOException
	...
	/**
	 * Reads bytes.length bytes and is much faster than calling read()
	 * bytes.length times.
	 * 
	 * @param bytes
	 *            the reference of a byte array
	...
	/**
	 * Reads a byte from the stream. Returns -1 if end of file is reached &
...
//		_location = _location + dicom.length;
	 ...
	 * contain an image number. Can throw an exception if the stream was no
	 * valid DICOM file.
	 * 
	 ...
	 * 
	 * @param little_endian
	 *            true if little endian should be true
	 */
